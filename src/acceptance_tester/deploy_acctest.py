...
def create_rawrepo_oai_test_home(path, repository, clean):
    ...
    create_initialize_script(path, 'rawrepo-oai/formatter', 'rawrepo-oai/setmatcher')
...
def create_corepo_solr_test_home(path, repository, clean):
    ...
    build_dir = create_dir(os.path.join(path, 'build-folder'))
    checkout_svn(path, get_svn_url("solr-indexer-cloud/trunk/worker"), "solr-indexer-cloud-worker", clean)
    ...
    create_ini_file(config_script, {'solr-config': os.path.join(path, 'corepo-indexer/solr/target/corepo-indexer-solr-1.1-SNAPSHOT.zip'),
                                    'javascript': os.path.join(path, 'solr-indexer-cloud-worker/src/main/resources/javascript'),
                                    'loglevel': 'TRACE'})

    create_mvn_suite_runner(path, build_dir, config_script, repository, 'solr-indexer-cloud-worker', "corepo-indexer/solr")

    create_javascript_suite_runner(path, 'solr-indexer-cloud-worker')
    #create_javascript_suite_runner(path, 'corepo-indexer', 'solr-indexer-cloud-worker')
    create_initialize_script(path, 'solr-indexer-cloud-worker')
    #create_initialize_script(path, 'corepo-indexer', 'solr-indexer-cloud-worker')
    create_jsshell(path, 'solr-indexer-cloud-worker')
...
def create_initialize_script(home_dir, *mvn_folders):
...
def create_jsshell(home_dir, folder):
    ...
    with open(path, 'w') as fh:
        ...
        fh.write('#!/bin/sh\n')
        fh.write('# Script auto-generated by deploy_acctest_environment\n\n')
        fh.write('exec '+ folder + '/jsshell.sh "$@"\n')
